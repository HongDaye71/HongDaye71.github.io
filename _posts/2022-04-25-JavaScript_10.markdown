...
```
* Sync callback : 즉각적으로 실행되는 콜백함수 (ex.setTimeout에서 delayTime을 지정하지 않은 경우)
* Async callback : 실행시간을 예측할 수 없는 콜백함수 (ex.setTimeout에서 delayTime을 지정한 경우)


#### 1.3 Callback hell<br/>
콜백함수를 반복사용하여 코드의 들여쓰기 수준이 감당하기 힘들 정도로 깊어지는 현상. 여기서 반복사용은 하나의 콜백함수 안에서 계속해서 콜백을 반복하는 것을 말함.
